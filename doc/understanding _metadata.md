...
This document details what is contained in the various files inside the `fdri-discovery` repository, the formatting of the files' contents, and how the files are interconnected and used by the metadata model to build the data dependency graph and metadata API.

In particular, this document shows how files should be updated to handle a new dataset/method/parameter/argument etc, and how the data gets processed by the `dri-timeseries-processor`. 

**Note:** This repository is a work in progress. There may be some inconsistencies in naming conventions. Datasets for different networks are also at different levels of completeness.


## Registering datasets

Each dataset must be registered inside `TIMESERIES_IDS_<NETWORK>.csv`.

When registering a dataset in `TIMESERIES_IDS_<NETWORK>.csv`, the dataset name is added to the `TIMESERIES_ID` column. Datasets of variables that are not measured, but that must be derived from other data, such as `ALBEDO` or `PE`, do not have a `SENSOR_SLOT_ID` and the entry to this column is left blank in `TIMESERIES_IDS_<NETWORK>.csv`. 

For 'intermediate' datasets, whose data output we do not wish to store, but is required to calculate another dataset, such as the `SOLAR_ZENITH`, the `S3_BUCKET` and `S3_DATASET` entries are also left blank `TIMESERIES_IDS_<NETWORK>.csv`.
...
The dataset names/`TIMESERIES_IDS` can be formed from the following options:
...
`<NETWORK>_TS_ID_DEPENDENCIES.json` is a JSON file where each dataset that depends on another dataset is listed as a top level key, together with their dependencies, and the processing step required to obtain that dataset from it's dependencies. The dataset names are the same as those in the `TIMESERIES_IDS` column from `TIMESERIES_IDS_<NETWORK>.json`.

**Note:** Not every dataset inside `TIMESERIES_IDS_<NETWORK>.csv` will have a dataset dependency config. Datasets that are loaded from raw data and/or do not depend on any other datasets do not have a dependency config.

The value for each dataset key is a nested dictionary. At the top level, keys and values of this nested dictionary are:
- key: `"depends_on"`, value: a list of dataset dependencies.
...
- key: `"method"`, value: the name of a specific method, either a derivation method or aggregation method as seen in the `dri-timeseries-processor` repository. The value of this key value pair should match exactly the `name` specified by the class in the `dri-timeseries-processor` repository where the intended method is implemented.

**Note:** The `"derivation"` key and derivation methods of the `dri-timeseries-processor` repository are not the same entities and should not be confused. They are purely naming conventions that mean different things in their respective repositories. Generally a `"derivation"` key with a value of `"CALCULATE"` in the `fdri-discovery` repository corresponds to a `derivation` method in the `dri-timeseries-processor` repository.

Depending on the method, there may be an additional key: `"args"`. The value is another nested dictionary. They key/value pairs inside this dictionary will depend on the specifics of the method and may include further nested dictionaries. Some of the key-value pairs found in `"args"` include:
...
- key: `"start_/end_date"`: value: a string that gets converted to a date time object in the `dri-timeseries-processor` repository. used for `"AGGREGATE"` methods where only certain times of the day should be aggregated, such as the albedo.
...
These files are not specific to a given network.
...
For example, the dataset `COSMOS-ALIC1-PE_30MIN_PROCESSED` is obtained by applying the correction method `clip` to the raw data, `COSMOS-ALIC1-PE_30MIN_RAW`. The `clip` method is implemented in the `dri-timeseries-processor` repository. The dataset dependency config can be found in `COSMOS_TS_ID_DEPENDENCIES.json`: 
...
Recently some QC configs have been added to `FDRI_QC_CONFIGS.csv`.
...
When the `"derivation"` value for a given dataset dependency config in `<NETWORK>_TS_ID_DEPENDENCIES.json` is `"CALCULATE"` or `"AGGREGATE"`, a `"method"` is also specified, along with any required arguments, `"args"`.

The method specified is not implemented in this repository, but rather in the `dri-timeseries-processor` repository. The value of a `"method"` key in `<NETWORK>_TS_ID_DEPENDENCIES.json` should match the CLI `name` provided in method classes inside either `aggregation_methods.py` or `derivation_methods.py` from the `dri-timeseries-processor` repository, though is agnostic of lower/upper case.
...
**Note:** There are no additional `csv` files for the metadata model to handle regarding `AGGREGATE` or `CALCULATE` derivations, everything is handled in the dataset dependency configs in `<NETWORK>_TS_ID_DEPENDENCIES.json` and the `dri-timeseries-processor`, and methods do not need to be registered in a separate file as was done during the `PROCESS` stage.

**Note:** Most dataset dependency configs in `<NETWORK>_TS_ID_DEPENDENCIES.json` that use `CALCULATE` or `AGGREGATE` construct processed datasets from other processed datasets. Occasionally a raw dataset will be constructed from processed datasets, for example: 
...
**Note:** Datasets that are obtained using `CALCULATE` must register their calculation method in `MEASURES.csv`, in additional to the units of measurement of the dataset. This may be `UNITLESS`. Units must also be registered in `UNITS.csv`, and 

Depending on the method specified, the `"args"` are specified by yet another nested dictionary. For details see the section **Dataset dependency configs**.
