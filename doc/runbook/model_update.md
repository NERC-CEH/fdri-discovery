...
4. Add a testing framework to the dri-metadata-ingest-config repository that can be used to validate that the output of the ingester conforms to the recordspec schema. This may be able to make use of the SHACL validation rules that can be generated from the recordspec schema. Ideally the testing framework should take representative CSV / JSON inputs for each of the configured endpoints and verify that the output is both conformant to the recordspec schema and matches an expected output. Test inputs and expected outputs should be updated as the templates are modified.
